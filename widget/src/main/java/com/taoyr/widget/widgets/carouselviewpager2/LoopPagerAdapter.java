...
public abstract class LoopPagerAdapter<T> extends PagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    void prepareData() {
        ...
        mViewPager.getViewPager().addOnPageChangeListener(this);
        mViewPager.getViewPager().setCurrentItem(1,false);
    }
    ...
    public void onPageSelected(int position) {
        ...
        if (currentPosition == 0 && !mInTransition) {
            mInTransition = true;
            // 使得切换更平滑
            mViewPager.getViewPager().setCurrentItem(getCount() - 1, false);
            mViewPager.getViewPager().setCurrentItem(getCount() - 2);

            //mViewPager.setCurrentItem(getCount() - 2, false);
        } else if (currentPosition == getCount() - 1 && !mInTransition) {
            mInTransition = true;
            // 若当前为倒数第一张，设置页面为第二张
            mViewPager.getViewPager().setCurrentItem(0, false);
