 ...
 * 该类支持缩放，平移拖动。但是只能针对一些简单的布局，比如imageview。对于一些会抢占事件的复杂的嵌套
 * 布局，比如listview，scrollview，无能为力。
 */
...
//            }
    ...
    public class ScaleGestureListener implements
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            // 缩放view
            if (scale > 0.8) { // 0.8是个临界值，开源控件无法控制的bug，到0.5，0.6会鬼畜
                ViewHelper.setScaleX(ZoomLayout.this, scale);// x方向上缩放
                ViewHelper.setScaleY(ZoomLayout.this, scale);// y方向上缩放
            } else {
                scale = preScale = 0.8f;
            }
            LogMan.logError("scale: " + scale);
